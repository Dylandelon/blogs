...
* 遍历所有的训练样本 for i = 1 to m
    * 设置输入层的激励 set $a^{(1)} = x^{(1)}$
    * 执行向前扩散计算，算出所有的激励 perform forward propagation to compute $a^{(l)}$ for $l = 2, 3, ... , L$
    * 使用输出值，计算输出层的误差 using $y^{(i)}$, compute $\delta^{(L)} = a^{(L)} - y^{(i)}$
    * 使用反向扩散的方法计算每一层的误差 compute $\delta^{(L-1)}, \delta^{(L-2)}, ..., \delta^{(2)}$ by using back propagation of error term
