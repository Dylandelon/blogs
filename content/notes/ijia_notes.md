...
本节介绍几个关键数据结构，实际上在 i-jia 系统中数据结构的复杂度和数量还是比较多的。

### 入网设备表 － 表 1

入网设备表保存了所有的入网的物理设备列表。

```c
typedef struct DeviceInfoTable_t{
    struct DeviceInfoTable_t *Next;
    u16 Len;                // 保存在数据库中的数据长度
    u16 Index;              // 在数据库中的索引
    u8  extAddr[8];         // 设备的 MAC 地址
    u8  capabilities;       // App 端没有使用。网关用这个字段来标识出协调器
    u8  activeType;
    u8  EndPointCNT;
    u8  EndPointRspCNT;
}DeviceInfoTable_Typedef;
```
