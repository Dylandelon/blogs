Title: 基于协同过滤算法的推荐系统
Date: 2015-12-19 22:36
Modified: 2015-12-19 22:36
Slug: recommend-system
Tags: machine-learning
Authors: Joey Huang
Summary: 推荐系统有着广泛的应用，电影推荐，商品推荐等都用到推荐系统。本文介绍协同过滤算法的基本原理，进而理解推荐系统的实现原理。
...
* 选择合适的计算周期
  我们不一定在每次用户提交一个新的电影评分数据时就计算，而是一天甚至几天计算一次。这样的结果就是，只要数据库的读写分离做得好，即使我们用很有限的硬件资源，也可以完成预期的计算目标。因为我们的计算时效性要求不强，每次计算完，更新参数后，我们的推荐系统就变得更准确一点了。
* 使用分布式计算
* 使用量子计算机
  量子计算机的运算速度上比目前的计算机要提高 N 个数量级。
